...
This module provides a command-line interface for the Elliptic Curve Diffie-Hellman key exchange.
...
def generate_and_display(key_size: int, verbose: bool = False):
    """
    Generate curve parameters and display a simulation of the ECDH key exchange.
    ...
    # Generate curve parameters and simulate key exchange
...
def measure_performance(num_trials: int = 5):
    """
    Measure the performance of the ECDH implementation for different key sizes.
    
    Args:
        num_trials: Number of trials to run for each key size
    """
    ...
    for key_size in key_sizes:
        ...
        # Store performance measurements for this key size
        param_times = []
        key_gen_times = []
        shared_secret_times = []
        ...
        for i in range(num_trials):
            ...
            # Initialize ECDH with parameters
            ...
            # Measure shared secret computation
            start_time = time.time()
        ...
        results[key_size] = {
            'param_generation': {
                'mean': statistics.mean(param_times),
                'min': min(param_times),
                'max': max(param_times),
                'std_dev': statistics.stdev(param_times) if len(param_times) > 1 else 0
            },
            'key_generation': {
            ...
            'shared_secret': {
                'mean': statistics.mean(shared_secret_times),
                'min': min(shared_secret_times),
                'max': max(shared_secret_times),
                'std_dev': statistics.stdev(shared_secret_times) if len(shared_secret_times) > 1 else 0
            }
        }
    ...
    print("=" * 80)
    print(f"{'Key Size':^10} | {'Operation':^20} | {'Mean':^12} | {'Min':^12} | {'Max':^12} | {'Std Dev':^12}")
    print("-" * 80)
    
    for key_size in key_sizes:
        # Parameter generation row
        param_stats = results[key_size]['param_generation']
        print(f"{key_size:^10} | {'Parameter Gen':^20} | {param_stats['mean']:^12.3f} | {param_stats['min']:^12.3f} | {param_stats['max']:^12.3f} | {param_stats['std_dev']:^12.3f}")
        
        # Key generation row
        key_stats = results[key_size]['key_generation']
        print(f"{' ':^10} | {'Key Generation':^20} | {key_stats['mean']:^12.3f} | {key_stats['min']:^12.3f} | {key_stats['max']:^12.3f} | {key_stats['std_dev']:^12.3f}")
        
        # Shared secret row
        shared_stats = results[key_size]['shared_secret']
        print(f"{' ':^10} | {'Shared Secret':^20} | {shared_stats['mean']:^12.3f} | {shared_stats['min']:^12.3f} | {shared_stats['max']:^12.3f} | {shared_stats['std_dev']:^12.3f}")
    ...
    # Also create a simplified table for Task 2 requirement
    ...
    for key_size in key_sizes:
        key_time = results[key_size]['key_generation']['mean']
        shared_time = results[key_size]['shared_secret']['mean']
...
def simulate_exchange():
    ...
    key_size = 128  # Default to 128-bit for this simulation
