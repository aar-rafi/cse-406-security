...
def parse_args():
    ...
    action.add_argument("--both", action="store_true", help="Encrypt the input text and then decrypt it")
...
def main():
    ...
    if args.encrypt:
        ...
        print("\n=== ENCRYPTION PHASE ===")
        iv, ciphertext = encrypt(args.input, args.key, args.verbose)
        
        full_hex = (iv + ciphertext).hex()
        
        print("\n=== DECRYPTION PHASE ===")
        decrypted_text = decrypt(full_hex, args.key, None, args.verbose)
        
        print("\n=== FINAL RESULT ===")
        print(f"Original input: '{args.input}'")
        print(f"Decrypted text: '{decrypted_text}'")

        if args.input == decrypted_text:
            print("SUCCESS: Decryption successfully recovered the original text")
        else:
            print("WARNING: Decrypted text doesn't match the original input")
